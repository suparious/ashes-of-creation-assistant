...
<#
.SYNOPSIS
    Build and push MyAshes.ai Backend Docker image to GitHub Container Registry

.DESCRIPTION
    Builds the MyAshes.ai FastAPI backend application Docker image and optionally pushes to GitHub Container Registry

.PARAMETER GitHub Container RegistryUsername
    GitHub Container Registry username (default: suparious)

.PARAMETER ImageName
    Docker image name (default: myashes-backend)
...
param(
    ...
    [Parameter(Mandatory = $false)]
    [string]$ImageName = "myashes-backend",
...
Write-ColorOutput "`n=== MyAshes.ai Backend - Docker Image Builder ===" $colors.Header
Write-ColorOutput "Building Docker image for FastAPI backend application`n" $colors.Info
...
Write-ColorOutput "This may take 5-10 minutes (installing ML dependencies)" $colors.Info
Write-Host ""
...
if ($LASTEXITCODE -ne 0) {
    ...
    Write-ColorOutput "✅ Python runtime verified" $colors.Success
}
Write-Host ""
...
if (-not $Push) {
    ...
    Write-Host "     docker run --rm -p 8000:8000 -e DEBUG=true $fullImageName" -ForegroundColor Yellow
    Write-Host "     Open http://localhost:8000/docs" -ForegroundColor Yellow
    Write-Host ""
